...
<script setup lang="ts">
	...
	onMounted(() => {
		watchEffect(() => {
			...
			const render = () => {
				if (renderedStyles.tx) renderedStyles.tx.current = mouse.x;
				if (renderedStyles.ty)renderedStyles.ty.current = mouse.y;

				for (const key in renderedStyles) {
					if (renderedStyles[key]) {
			...
			const onMouseMove = () => {
				if (renderedStyles.tx) renderedStyles.tx.previous = renderedStyles.tx.current = mouse.x;
				if (renderedStyles.ty) renderedStyles.ty.previous = renderedStyles.ty.current = mouse.y;
