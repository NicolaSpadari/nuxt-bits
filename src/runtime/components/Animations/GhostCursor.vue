<template>
	<div ref="containerRef" class="pointer-events-none absolute inset-0" :class="[className]" :style="mergedStyle" />
</template>
...
<script setup lang="ts">
	...
	const setup = () => {
		...
		const animate = () => {
			...
			for (let i = 0; i < N; i++) {
				...
				if (trailBufRef.value[srcIdx])
					arr[i]?.copy(trailBufRef.value[srcIdx]);
			}

			if (mat.uniforms.iOpacity) {
	...
	watch(
		() => props.brightness,
		() => {
			if (materialRef.value && materialRef.value.uniforms.iBrightness) {
	...
	watch(
		() => props.edgeIntensity,
		() => {
			if (materialRef.value && materialRef.value.uniforms.iEdgeIntensity) {
