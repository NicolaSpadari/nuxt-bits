...
<script setup lang="ts">
	...
	const scrollTransforms = computed(() => {
		return props.texts.map((_, index) => {
			...
			return `${wrap(-(singleWidth || 1), 0, baseX.value[index] || 0)}px`;
		});
	});
	...
	onMounted(async () => {
		...
		baseX.value = Array.from({ length: props.texts.length }).fill(0) as number[];
		copyWidths.value = Array.from({ length: props.texts.length }).fill(0) as number[];
		calculatedCopies.value = Array.from({ length: props.texts.length }).fill(15) as number[];
		directionFactors.value = Array.from({ length: props.texts.length }).fill(1) as number[];
