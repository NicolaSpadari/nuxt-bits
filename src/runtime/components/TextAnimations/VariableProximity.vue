  ...
  <script setup lang="ts">
		...
		const parseSettings = (settingsStr: string) => {
			...
			settingsStr.split(",").forEach((s) => {
				...
				if (parts.length === 2) {
					...
					const value = Number.parseFloat(parts[1] ?? "0");
	...
	const getGlobalLetterIndex = (wordIndex: number, letterIndex: number) => {
		...
		for (let i = 0; i < wordIndex; i++) {
			globalIndex += words.value[i]?.length ?? 0;
		}
	...
	const animationLoop = () => {
		...
		const newSettings = Array.from({ length: letterElements.value.length }).fill(props.fromFontVariationSettings);
		...
		interpolatedSettings.value = newSettings as string[];

		letterElements.value.forEach((letterEl, index) => {
			if (interpolatedSettings.value[index]) {
