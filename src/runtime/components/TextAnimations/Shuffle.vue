<template>
  ...
  <script setup lang="ts">
	...
	const scrollTriggerStart = computed(() => {
		...
		const mv = mm ? Number.parseFloat(mm?.[1] || "0") : 0;
	...
	const play = () => {
		...
		const tl = gsap.timeline({
			...
			onRepeat: () => {
				...
				gsap.set(strips, { x: (i, t: HTMLElement) => Number.parseFloat(t.getAttribute("data-start-x") || "0") });
		...
		const addTween = (targets: HTMLElement[], at: number) => {
			tl.to(
				targets,
				{
					x: (i, t: HTMLElement) => Number.parseFloat(t.getAttribute("data-final-x") || "0"),
		...
		if (props.animationMode === "evenodd") {
			...
			strips.forEach((strip) => {
				...
				tl.to(
					strip,
					{
						x: Number.parseFloat(strip.getAttribute("data-final-x") || "0"),
						duration: props.duration,
						ease: props.ease,
						force3D: true
					},
