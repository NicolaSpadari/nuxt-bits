...
<script setup lang="ts">
	...
	const setup = () => {
		...
		const loop = () => {
			...
			if (material.uniforms.uRot) (material.uniforms.uRot.value as THREE.Vector2).set(c, s);

			const cur = pointerCurrentRef.value;
			const tgt = pointerTargetRef.value;
			const amt = Math.min(1, dt * pointerSmoothRef.value);
			cur.lerp(tgt, amt);
			if (material.uniforms.uPointer) (material.uniforms.uPointer.value as THREE.Vector2).copy(cur);
	...
	watch(
		...
		() => {
			...
			const toVec3 = (hex: string) => {
				...
				if (!h || h.length < 3) return new THREE.Vector3(0, 0, 0);
				const v
					= h.length === 3
