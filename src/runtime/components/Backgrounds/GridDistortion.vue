<template>
	<div ref="containerRef" class="w-full h-full overflow-hidden" :class="[props.className]" />
</template>
		...
		const animate = () => {
			...
			for (let i = 0; i < size * size; i++) {
				...
				if (data[rIndex] !== undefined) {
					data[rIndex] *= props.relaxation;
				}

				if (data[gIndex] !== undefined) {
					data[gIndex] *= props.relaxation;
				}
			}
