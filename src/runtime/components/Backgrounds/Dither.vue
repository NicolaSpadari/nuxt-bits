...
`;
	...
	const update = (t: number) => {
		...
		if (props.enableMouseInteraction) {
			...
			if (currentMouse[0] && targetMouse[0]) currentMouse[0] += smoothing * (targetMouse[0] - currentMouse[0]);
			if (currentMouse[1] && targetMouse[1]) currentMouse[1] += smoothing * (targetMouse[1] - currentMouse[1]);
