...
<script setup lang="ts">
	...
	const setupFn = () => {
		...
		const handleResize = () => {
			...
			if (material.uniforms.uResolution) material.uniforms.uResolution.value.set(w, h);
		};
		...
		const animate = () => {
			...
			if (material.uniforms.uTime) material.uniforms.uTime.value = (performance.now() - startTime) * 0.001;
