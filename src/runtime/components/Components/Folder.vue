		...
		<div :class="folderClass" :style="folderStyle" @click="handleClick">
			...
			>
				...
				<div v-for="(item, i) in papers" :key="i" :class="getPaperClasses(i)" :style="getPaperStyle(i)">
					<slot :name="`item-${i + 1}`" :item="item" :index="i" :is-open="open">
						{{ item }}
					</slot>
				</div>
				<div :class="frontClass" :style="{ backgroundColor: props.color }" />
				<div :class="rightClass" :style="{ backgroundColor: props.color }" />
			</div>
		</div>
	</div>
</template>
