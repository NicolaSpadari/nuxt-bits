<template>
	<div
		ref="scrollerRef"
		class="relative w-full h-full overflow-y-auto overflow-x-visible" :class="[className]"
		:style="{
	...
	>
		<div class="px-20 pt-[20vh] pb-200 min-h-screen scroll-stack-inner">
			<slot />
		...
		cardsRef.value.forEach((card, i) => {
			...
			if (props.blurAmount) {
				...
				for (let j = 0; j < cardsRef.value.length; j++) {
					const jCardTop = cardsRef.value[j]?.offsetTop ?? 0;
