<template>
	<nav :class="containerClassName" :style="style" aria-label="Main navigation">
		<div
			class="bubble logo-bubble inline-flex items-center justify-center rounded-full bg-white shadow-[0_4px_16px_rgba(0,0,0,0.12)] pointer-events-auto h-12 md:h-14 px-4 md:px-8 gap-2 will-change-transform"
			aria-label="Logo"
			:style="{
				background: menuBg,
				minHeight: '48px',
		...
		<button
			type="button"
			class="bubble toggle-bubble menu-btn inline-flex flex-col items-center justify-center rounded-full bg-white shadow-[0_4px_16px_rgba(0,0,0,0.12)] pointer-events-auto w-12 h-12 md:w-14 md:h-14 border-0 cursor-pointer p-0 will-change-transform" :class="[
				isMenuOpen ? 'open' : '',
			]"
	...
	<div
		v-if="showOverlay"
		ref="overlayRef"
		class="bubble-menu-items inset-0 flex items-center justify-center pointer-events-none z-[1000]" :class="[
			useFixedPosition ? 'fixed' : 'absolute',
		]"
		:aria-hidden="!isMenuOpen"
	>
		<ul
			class="pill-list list-none m-0 px-6 w-full max-w-[1600px] mx-auto flex flex-wrap gap-x-0 gap-y-1 pointer-events-auto"
			role="menu"
		...
		>
			...
			>
				<a
					...
					class="pill-link w-full rounded-[999px] no-underline bg-white text-inherit shadow-[0_4px_14px_rgba(0,0,0,0.10)] flex items-center justify-center relative transition-[background,color] duration-300 ease-in-out box-border whitespace-nowrap overflow-hidden"
					:style="{
