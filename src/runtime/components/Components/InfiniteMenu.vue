<template>
	<div class="relative w-full h-full">
		...
		<template v-if="activeItem">
			<h2
				class="select-none absolute font-black text-6xl top-1/2 text-white transition-all duration-500 ease-in-out hidden xl:block" :class="[
					isMoving
			...
			<p
				class="select-none absolute top-1/2 text-2xl text-white/80 transition-all ease-in-out hidden xl:block" :class="[
			...
			<div
				class="absolute left-1/2 z-10 grid place-items-center bg-purple-600 rounded-full cursor-pointer border-4 border-black transition-all ease-in-out" :class="[
					isMoving ? 'pointer-events-none opacity-0 duration-100' : 'opacity-100 pointer-events-auto duration-500',
				]"
				:style="{
					width: '60px',
			...
			>
				<p class="relative text-white text-2xl select-none" :style="{ top: '2px' }">
					&#x2197;
				</p>
			</div>
		</template>
	...
	class Geometry {
		...
		subdivide(divisions = 1): this {
			...
			for (let div = 0; div < divisions; ++div) {
				const newFaces = Array.from({ length: f.length * 4 });
	...
	class InfiniteGridMenu {
		...
		private init() {
			...
			this.discVAO = makeVertexArray(
				...
				[
					[
						makeBuffer(this.gl, this.discBuffers.vertices ?? new Float32Array(), this.gl.STATIC_DRAW)!,
						this.discLocations.aModelPosition as number,
						3
					],
					[makeBuffer(this.gl, this.discBuffers.uvs ?? new Float32Array(), this.gl.STATIC_DRAW)!, this.discLocations.aModelUvs as number, 2]
				],
		...
		private render() {
			...
			this.gl.uniformMatrix4fv(this.discLocations.uWorldMatrix as WebGLUniformLocation, false, this.worldMatrix);
			this.gl.uniformMatrix4fv(this.discLocations.uViewMatrix as WebGLUniformLocation, false, this.camera.matrices.view);
			this.gl.uniformMatrix4fv(this.discLocations.uProjectionMatrix as WebGLUniformLocation, false, this.camera.matrices.projection);
			this.gl.uniform3f(
			...
			this.gl.drawElementsInstanced(
				this.gl.TRIANGLES,
				this.discBuffers.indices?.length ?? 0,
