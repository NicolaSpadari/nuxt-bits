<template>
	<div class="top-[1em] left-0 md:left-auto z-[1000] absolute w-full md:w-auto">
		<nav
			class="w-full md:w-max flex items-center justify-between md:justify-start box-border px-4 md:px-0" :class="[className]"
			aria-label="Primary"
			:style="cssVars"
		>
			<component
			...
			>
				<img ref="logoImgRef" :src="logo" :alt="logoAlt" class="block w-full h-full object-cover">
			</component>
...
<script setup lang="ts">
	...
	watch(
		() => props.items,
		(items) => {
			circleRefs.value = Array.from({ length: items.length }).fill(null) as Array<HTMLSpanElement | null>;
			tlRefs.value = Array.from({ length: items.length }).fill(null) as Array<gsap.core.Timeline | null>;
			activeTweenRefs.value = Array.from({ length: items.length }).fill(null) as Array<gsap.core.Tween | null>;
		},
		{ immediate: true }
	);
